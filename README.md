...
**Data status: everything in this repo's evaluation is synthetic.**
`main.py`'s entire pipeline -- burn-in, all 4 drift scenarios, the
radiation-damage case study, throughput benchmark -- runs on synthetic
data generated by `src/stream_loader.py` and `src/drift_sim/*.py`. The
ONE exception is `scripts/real_data_touchpoint.py`, which fits
`residual.py`'s regression on one real CMS Open Data file (record 30558)
-- narrow in scope (the regression/residual machinery only, not the VAE,
not any detector, not the full pipeline; see `data/README.md`). Full
real-data validation of the detectors and scenarios above is planned for
a follow-on full paper (not this short paper), not yet started.
...
- **BOCPD underperforms CUSUM on continuous, non-resetting drift
  ...
  accumulation has no equivalent escape hatch.
...
- **One-sided CUSUM for masked-channel: real improvement, smaller than
  ...
  checked (e.g. seed=0: 0.93 -> 0.53 miss_rate; seed=1: 0.80 -> 0.73),
  but nowhere near the initial "near-total detection" estimate. Reported
  here as a genuine methodological finding, not a solved problem: a more
  sensitive, correctly-directed detector still trades against false-alarm
  risk under a strict single-shot detection semantics, and that trade-off
  is real, not a bug to engineer away. TODO: run across >=5 seeds for a
  stable estimate before citing a specific miss_rate range in the paper.
